...
if run or search_ticker or enable_auto:
    ...
    if results:
        ...
        sentiment = "🚀 BULLISH" if breadth > 60 else ("⚠️ CAUTION" if breadth > 40 else "💀 BEARISH")
        s_color = "#10b981" if breadth > 60 else ("#f59e0b" if breadth > 40 else "#ef4444")
        ...
        for item in sorted(results, key=lambda x: x["sc"], reverse=True):
            ...
            with c3:
                ...
                lot_size = int(risk_money / (entry_p - sl_p) / 100) if (entry_p - sl_p) > 0 else 0
